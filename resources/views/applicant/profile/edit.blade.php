    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-lg shadow-md">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-xl font-semibold text-gray-900">Edit Data Profil RPL</h2>
                <p class="text-gray-600 mt-1">Perbarui data profil Anda</p>
            </div>

            <form action="{{ route('applicant.profile.update') }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="gender" name="gender"
                                ...
                                <option value="male" {{ old('gender', $applicant->gender) === 'male' ? 'selected' : '' }}>
                                    Laki-laki</option>
                                <option value="female"
                                    {{ old('gender', $applicant->gender) === 'female' ? 'selected' : '' }}>Perempuan
                                </option>
                            </select>
                            @error('gender')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="address" name="address" rows="3"
                                ...
                                required>{{ old('address', $applicant->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="tel" id="phone_number" name="phone_number"
                                value="{{ old('phone_number', $applicant->phone_number) }}" placeholder="08xxxxxxxxxx"
                            ...
                            <select id="graduate_category_id" name="graduate_category_id"
                                ...
                                @foreach ($graduateCategories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('graduate_category_id', $applicant->graduate_category_id) == $category->id ? 'selected' : '' }}>
                            ...
                            <select id="study_program_id" name="study_program_id"
                                ...
                                @foreach ($studyPrograms as $program)
                                    <option value="{{ $program->id }}"
                                        {{ old('study_program_id', $applicant->study_program_id) == $program->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea id="motivation" name="motivation" rows="4"
                        ...
                        placeholder="Jelaskan motivasi dan tujuan Anda mengikuti program RPL...">{{ old('motivation', $applicant->motivation) }}</textarea>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                            ...
                            <p class="font-medium text-red-600">{{ $applicant->registration_number }}</p>
                        </div>
                        <div>
                            ...
                            <p class="font-medium">
                                @switch($applicant->application_status)
                                    ...
                                    @case('submitted')
                                        <span class="text-blue-600">Diajukan</span>
                                    @break

                                    @case('under_review')
                                        <span class="text-yellow-600">Dalam Review</span>
                                    @break
                        ...
                        <div>
                            ...
                            <p class="font-medium">{{ $applicant->created_at->format('d/m/Y') }}</p>
                        </div>
                    </div>
                </div>
